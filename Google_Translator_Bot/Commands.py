        ...
        reply_markup = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton
                        (
                            text = "❤️ Faire un don️", url = "https://t.me/A_liou"
                         )
                ],
                [
                    InlineKeyboardButton
                        (
                            text = "📖 Chaînes", callback_data="credits"
                        )
                ]
            ]
        )
...
async def broadcast_(c, m):
    ...
    out = await m.reply_text(
        text = f"Diffusion lancée ! Vous serez averti avec le fichier journal lorsque tous les utilisateurs seront avertis."
    )
    ...
    if failed == 0:
        await m.reply_text(
            text=f"diffusion achevée en `{completed_in}`\n\nUtilisateurs: {total_users}.\nTotal done {done}, {success} success and {failed} failed.",
            quote=True
        )
    else:
